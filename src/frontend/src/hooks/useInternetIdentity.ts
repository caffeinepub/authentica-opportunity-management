...
export function InternetIdentityProvider({
  ...
  // Store authClient and createOptions in refs so changes never trigger effect re-runs
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // NO finally block -- it would overwrite "success" with "idle"
    })();
    return () => {
      cancelled = true;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally empty -- createOptionsRef holds the value without triggering re-runs

  const login = useCallback(() => {
    const client = authClientRef.current;
    if (!client) {
      setStatus("loginError");
      setError(new Error("AuthClient not ready yet"));
      return;
    }
