...
export function InternetIdentityProvider({
  ...
  const login = useCallback(() => {
    ...
    // Only block login if we already have a valid delegation AND status is success
    ...
    void (async () => {
      try {
        ...
        if (isAuthenticated) {
          ...
          // CRITICAL: set success when a stored session is found -- do NOT fall through to "idle"
          setStatus("success");
        } else {
          setStatus("idle");
        }
