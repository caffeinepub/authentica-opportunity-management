...
export function InternetIdentityProvider({
  ...
  const login = useCallback(() => {
    ...
    // If there's already a valid stored delegation, surface it as success
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // Only reset to idle if we didn't find an authenticated session
        if (!cancelled && !foundAuth) setStatus("idle");
      }
    })();
    return () => {
      cancelled = true;
